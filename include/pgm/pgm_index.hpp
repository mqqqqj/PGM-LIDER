...
#define PGM_ADD_EPS(x, epsilon, size) ((x) + (epsilon) + 2 >= (size) ? (size) : (x) + (epsilon) + 2)
    ...
    public:
        ...
        void saveIndex(const std::string &location)
        {
            ...
            output.write(reinterpret_cast<const char *>(&first_key), sizeof(K));
            size_t segments_size = segments.size();
            output.write(reinterpret_cast<const char *>(&segments_size), sizeof(size_t));
            for (size_t i = 0; i < segments_size; i++)
            {
                output.write(reinterpret_cast<const char *>(&segments[i].key), sizeof(K));
                output.write(reinterpret_cast<const char *>(&segments[i].slope), sizeof(Floating));
                output.write(reinterpret_cast<const char *>(&segments[i].intercept), sizeof(int32_t));
            }
            size_t levels_offsets_size = levels_offsets.size();
            output.write(reinterpret_cast<const char *>(&levels_offsets_size), sizeof(size_t));
            for (size_t i = 0; i < levels_offsets_size; i++)
            {
                output.write(reinterpret_cast<const char *>(&levels_offsets[i]), sizeof(size_t));
            }
            output.close();
        }
    };
